...
def generate_initialize_subprotocol(doc):
    ...
    water = sbol3.Component(
        "MLQ water", "https://identifiers.org/pubchem.substance:24901740"
    )
 ...
 protocol to initialize transfer plasmid and ethanol to a MPE container for ethanol washes
    ...
    water_container = protocol.primitive_step(
        "EmptyContainer",
        specification=labop.ContainerSpec(
            "WATER For cleanup after ethanol washing steps",
...
def generate_protocol():
    ...
    PROTOCOL_NAME = "PCR purification"
...
This DNA cleanup/purification protocol is to be executed using 2 HAMILTON modules. They are:
   ...
   *# HHS/Heater shaker - to shake the final product at 30 celcius (necessary final step)    
    ...
    plan_mapping = serialize_sample_format(
            xr.DataArray(
                ...
                coords={
                    ...
                    f"source_{Strings.LOCATION}": ["A1", "A2", "A3", "A4", "A5", "A6", "A7", "A8"],
                    f"target_{Strings.CONTAINER}": [initialization.output_pin("shaking_incubator").name],
                    f"target_{Strings.LOCATION}": ["A1", "A2", "A3", "A4", "A5", "A6", "A7", "A8"],
    ...
    plan = labop.SampleMap(
        sources=[initialization.output_pin("PLASMID_container")][initialization.output_pin("Ethanol_container")][initialization.output_pin("MPE_container")][initialization.output_pin("water_container")],
        targets=[initialization.output_pin("MPE_container")][initialization.output_pin("shaking_incubator")]
        values=plan_mapping,
    )
...
#a sub-protocol should be made to represent this step
    ...
    MPE_subprotocol = generate_MPE_subprotocol(doc)
...
#here the ethanol would already be inside the MPE2 and the pressure pump would be activated
#pushing the ethanol through the collumns inside the container and finishing the first washing step
    def generate_MPE_subprotocol(doc):
        protocol = labop.Protocol("MPE_overpressure")
        doc.add(protocol)
    ...
    #here the ethanol would already be inside the MPE2 plate and the pressure pump would be activated
    #pushing the ethanol through the collumns inside the container and finishing the second washing step
   def generate_MPE_subprotocol(doc):
        protocol = labop.Protocol("MPE_overpressure")
        doc.add(protocol)
    ...
    "Incubate",
    location= shaking_incubator.output_pin("samples"),
    duration=sbol3.Measure(3, OM.min),
    temperature=sbol3.Measure(30, OM.degree_Celsius),
    shakingFrequency=sbol3.Measure(250, None),
    ) 
